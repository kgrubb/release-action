...
if ! curl --fail -sS -X POST https://api.github.com/repos/"$GITHUB_REPOSITORY"/releases -H "Accept: application/vnd.github.v3+json" -H "Authorization: token $GITHUB_TOKEN" -H "Content-Type: application/json" -d "$DATA"; then
